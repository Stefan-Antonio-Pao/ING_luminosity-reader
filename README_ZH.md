...
v3.1.0 真实可用、已端到端跑通的功能:

- **拍页伴读。** 单张拍照、对齐取景框,App 给出一段语音化的
  "夸赞 → 详细解释 → 拓展提问",紧扣画面与文字。
- **静默 OCR 校准(v3.1.0)。** 每张照片都先经过版面归一化(阅读顺序、双页拆分、
  页码剔除)、置信度质量门控、端侧 Gemma 保守修正、以及保护数字 / 人名 / 引号内容的
  确定性校验器。全程静默:烂文本被默默修好或默默丢弃改聊画面——**绝不会弹"请重拍"
  打断孩子**。
- **真·离线词典(v3.1.0)。** `lookup_word` 接入随包 ~31 MB SQLite 词库——
  WordNet 3.1(英)+ CC-CEDICT(中),真实释义、按年龄段裁剪、完全离线。
...
**运行时流程。** 从 **v3.1.0** 起,每个 OCR 结果都会先经过一条**静默校准管线**——
版面归一化、置信度质量门控、Gemma 保守修正、以及保护数字 / 人名 / 引号内容的确定性
校验器。对孩子全程不可见:烂文本被默默修好或默默丢弃(伙伴自然改聊画面),绝无错误
提示打断。

```mermaid
flowchart TD
    A[CameraX · 单次拍照] --> B[ML Kit 端侧 OCR + 图像打标]
    B --> N[LayoutNormalizer · 阅读顺序 / 双页拆分 / 页码剔除]
    N --> Q{OcrQualityGate · 置信度或启发式}
    Q -- "高(≥0.88)" --> C
    Q -- "中(0.40–0.88)" --> X[OcrCorrectionStage · Gemma 修明显 OCR 错误,JSON 约束]
    Q -- "不可信(<0.40)" --> Y[静默丢弃文本 · 改聊画面]
    X --> V{OcrCorrectionValidator · 数字 / 人名 / 引号受保护}
    V -- "通过" --> C
    V -- "拒绝 → 保留原文" --> C
    Y --> C{AgentOrchestrator · 模型与复杂度策略}
    ...
    H --> I[模型产出最终适龄回应]
    E -- "非法 / 静默丢 / 超时 / 没调" --> F
    F -- "模型也不可用" --> T[TemplateReading · 确定性模板,绝不崩]
...
- **`classify_scene(image_labels, ocr_text)`** —— *完整可用。* 判断拍到的是绘本页还是
  眼前物品,据此走伴读或讲解物品。
- **`lookup_word(term)`** —— *自 v3.1.0 起完整可用。* 接入**随包真实离线词库**:
  WordNet 3.1(英,约 14.7 万词条)+ CC-CEDICT(中,约 18.8 万词条),构建期由
  `scripts/build_dict.py` 编译为 ~31 MB SQLite 随 APK 打包。释义先按年龄段裁剪再给孩子
  (幼儿只留一句,学龄给完整释义);查不到时优雅降级为适龄的"我们一起看看"——
  **模型绝不编造释义**。
...
flowchart LR
    ...
    SE -. "实现者" .-> ORCH[AgentOrchestrator · 模型策略 + 四层降级]
    ORCH --> FCE[FunctionCallingEngine · 手动工具循环]
    ORCH --> TSE[TwoStagePipelineEngine · 降级脊梁]
    ORCH --> TPL[TemplateReading · 确定性最终兜底]
    FCE --> TR["tools:classify_scene / lookup_word / read_aloud"]
    TR --> DICT["SqliteOfflineDictionary · WordNet + CC-CEDICT,随包"]
...
超时 / 校验不过 / 模型干脆没调)都会**自动回退到两阶段纯文本管线**,App 绝不崩。
从 v3.1.0 起降级链共**四层**:函数调用 → 两阶段文本 → 确定性模板伴读(模型本身不可用时)
→ 图像观察引导。函数调用是亮点;**可靠的基线由降级保证**。

**静默 OCR 校准(v3.1.0,诚实)。** 绘本拍照的现实是:角度歪、卧室光、双页摊开。校准
管线修能修的、**从不唠叨**:版面重排(双页先左后右、页码剔除)、中等置信度文本走 JSON
约束的保守修正("litt1e → little" 可以;重写故事不行)、确定性校验器拒绝任何碰数字 /
人名 / 引号内容的"修正"(被拒就退回 OCR 原文)。低于信任下限的文本被静默丢弃,伙伴
自然改聊画面。质量信号只进 logcat,绝不打扰孩子。在自制 8 类测试集(清晰 / 弱光 / 弯曲 /
双页 / 气泡等)上,清晰页字符错误率 0–6%,页码全部正确剔除。
...
仓库已包含编译好的词典 `app/src/main/assets/dict/lumi_dict.db`(~31 MB,
WordNet 3.1 + CC-CEDICT),正常构建无需额外步骤。如需从原始数据源重建
(例如更新 CC-CEDICT 快照):
...
cd ..
python scripts/build_dict.py   # 输出 app/src/main/assets/dict/lumi_dict.db
```
...
### v3.1.0 —— 静默 OCR 校准 + 真·离线词典(仅后端,UI 零改动)

- **新增** `:core/ocr` 静默 OCR 校准管线:
  - `LayoutNormalizer` —— 绘本阅读顺序(自上而下、双页摊开先左后右)、页码剔除、
    倾斜装饰字过滤;
  - `OcrQualityGate` —— 置信度门控(≥0.88 直接用 / 0.40–0.88 修正 / <0.40 静默丢弃),
    且 **ML Kit 不给可用置信度时自动退回启发式判定**(旧 Play 服务路径会报 0);
  - `OcrCorrectionStage` —— Gemma 以 JSON 约束的结构化生成修明显 OCR 错误(不是工具链),
    解析全程防御式:坏 JSON / 截断 / 模型异常一律降级保留原文,绝不崩;
  - `OcrCorrectionValidator` + `ProtectedTokenDetector` —— 确定性护栏:拒绝任何碰
    数字 / 人名 / 日期 / 引号内容的"修正",拒绝重写与扩写(长度与词级 diff 上限),
    并核验模型声明的修改确实存在于原文(反幻觉 diff)。
- **校准全程静默**:无重拍提示、无错误气泡——不可信文本被默默修复或默默丢弃
  (伙伴改聊画面);质量信号只进 logcat。
- **新增** `lookup_word` 真·离线词典:WordNet 3.1(英,约 14.7 万词条,WordNet License)
  + CC-CEDICT(中,约 18.8 万词条,CC BY-SA 4.0,MDBG 发布),由 `scripts/build_dict.py`
  编译为 ~31 MB SQLite 随包;释义按年龄段裁剪、未命中优雅降级;并有 JVM 与真机双重
  **硬验证**测试(与生产共用同一条查询,断言 `caterpillar` / `毛毛虫` 返回真实词条)。
- **新增** 第四层降级:函数调用与两阶段文本管线都失败时(如模型尚未下载),确定性
  `TemplateReading` 仍能念出页面文字并提一个适龄问题——App 永远不会"死路"。
- **新增** 自制(无版权、脚本渲染)8 类绘本 OCR 测试集(`testdata/`)与真机指标测试
  (CER/WER、页码剔除、受保护词存活、置信度可用性)。
- **OCR 结构更丰富**:`OcrLine` 携带 bounding box、角点、角度、行级语种与页面区域;
  `MlKitOcrService` 不再在 ML Kit 缺置信度时伪造 1.0。
- **未改动**:UI、三工具集(仍然恰好 3 个)、语言 / 年龄段设置、TTS、语音输入、家长区;
  模型仍首启下载(不进 APK)。
