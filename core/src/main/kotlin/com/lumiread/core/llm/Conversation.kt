 ...
 * 单轮对话句柄。
 *
 * LiteRT-LM 0.12 native 层"一个 Engine 一个活跃 session"的硬约束让"长期复用同一个 Conversation
 * 跑多轮"不可行。`com.lumiread.core.pipeline.ChatSession` 改为**每轮开一个新 Conversation,
 * 收完即 close**,把历史拼进 prompt 文本。所以这个接口本质上退化为"一次性"句柄;
 * [close] 必须在每轮 finally 里调用。
 *
 * 实现:
