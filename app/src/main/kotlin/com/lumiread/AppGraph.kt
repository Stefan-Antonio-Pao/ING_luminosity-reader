 ...
 * 进度:
 * - :全 Fake。
 * - :OCR + Label 切到 ML Kit;LLM/TTS 仍 Fake。
 * - :LLM 切到 `Gemma4Engine`(LiteRT-LM 0.12.0);TTS 仍 Fake。
 * - **(当前):TTS 切到 `SherpaTtsEngine`(sherpa-onnx 1.13.2 + vits-melo-tts-zh_en),
 * 两者都为真 → `isFakeMode = false` → 红色水印消失。**
 *
